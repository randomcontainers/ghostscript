...
The same images can also be pulled as `randomcontainers.com/ghostscript`.

Render every page of a PDF to a 150 dpi PNG:
...
docker run --rm --user "$(id -u):$(id -g)" -v "$PWD:/work" --entrypoint ps2pdf \
  ghcr.io/randomcontainers/ghostscript input.ps output.pdf
```
...
docker run --rm --user "$(id -u):$(id -g)" -v "$PWD:/work" \
  ...
  ghcr.io/randomcontainers/ghostscript -sDEVICE=pdfwrite -o clean.pdf untrusted.pdf
```
